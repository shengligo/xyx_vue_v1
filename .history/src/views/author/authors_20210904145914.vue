...
export default {
  ...
  mounted() {
    this.getEdit();
    console.log("object");
    console.log( new Date().toLocaleDateString()+ new Date().toTimeString());
  },
  methods: {
    getEdit() {
      ...
      // 配置 server 接口地址
      this.editor.config.uploadImgServer = 'http://10.1.2.85:8091/admin/upload/img' // 'http://10.1.2.85:8091/admin/upload/img'
    ...
    async saveEdit() {
      ...
      const createData = new Date().toLocaleDateString()+ new Date().toTimeString()
      var data = {
        id: 1,
        sortId: 1,
        title: this.title,
