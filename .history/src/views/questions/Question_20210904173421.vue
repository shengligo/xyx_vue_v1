<template>
  <div id="question">
    <div class="bodyCenter">
      <div class="bodyright">
        <div v-for="(item, index) in results" :key="index" class="rightbox" @click="goAnswer(item.quizId)">
          <div>
            <div class="answerer">
              <span>回答者：{{ item.answerer }}</span>
              <el-button type="primary" size="mini">去回答</el-button>
            </div>
            <div class="time">{{item.createTime}} 回答了：</div>
...
export default {
  ...
  methods: {
      ...
      await this.$ajax
        ...
        .then(res => {
          ...
          if (data.code === "000000") {
            ...
            this.condition.total = data.totalRecord || 0
            this.condition.pageNum = data.pageNum || 0
            this.condition.pageSize = data.pageSize || 0
            console.log(this.condition.total ,'this.condition.total ');
    ...
    goAnswer(id) {
      this.$router.push(`/quesDetail/${id}`)
    },
    handleCurrentChange() {
